...
def train_AE(experiment, hyp):
    ...
    training_generator = get_dataloader(encoder_type, hyp)   # Choose appropriate training dataset generator
    validation_generator = get_dataloader(encoder_type, hyp, split = 'val')   # Choose appropriate training dataset generator

    # **** BUILD NMF MODEL ******
    model = nn.DataParallel(NeuralMeshFlow(encoder_type = encoder_type, zdim=hyp['latent_len'], time=hyp['ToI'], tol=hyp['ToI'])).cuda()

    # Setup training helpers
    print("*********** SETUP TRAINING  ************")
    optimizer = optim.Adam(list(model.parameters()), lr = hyp['learning_rate'])
    scheduler = StepLR(optimizer, step_size=1, gamma=hyp['weight_decay'])
    os.makedirs(hyp['model_folder'],exist_ok=True)
    print("Models are being saved at :", hyp['model_folder'])
    ...
    for epoch in range(hyp['num_epochs']):
        for input,_,_ in training_generator:   # Point Cloud, model category, model name is given by training generator
            ...
            **** NOTE ******
            ...
            loss = (  hyp['training_weights'][0]*loss1 
                    + hyp['training_weights'][1]*loss2 
                    + hyp['training_weights'][2]*loss3)/(
                hyp['training_weights'][0]+hyp['training_weights'][1]+hyp['training_weights'][2])
        ...
        torch.save(model.state_dict(),hyp['model_folder']+'epoch_{}'.format(epoch))
...
def train_SVR(experiment, hyp):
    ...
    optimizer = optim.Adam(list(model.parameters()), lr = hyp['learning_rate'])
    scheduler = StepLR(optimizer, step_size=1, gamma=hyp['weight_decay'])
    os.makedirs(hyp['model_folder_SVR'],exist_ok=True)
    print("Models are being saved at :", hyp['model_folder_SVR'])
    if experiment is None: 
        print("Comet_ml logging is disabled, printing on terminal instead")
    print("*********** BEGIN TRAINING  ************")
    step=0
    for epoch in range(hyp['num_epochs']):
        ...
        print("Saving models after #Epoch :", epoch+1)  # Save models after every epoch
        torch.save(model.state_dict(),hyp['model_folder_SVR']+'epoch_{}'.format(epoch))
