...
if __name__ == '__main__':
    ...
    PRED_PATH, GDTH_PATH, OUTFILENAME, ROT, TYPE = get_configs(METHOD, TYPE)
    ...
    for cat in os.listdir(GDTH_PATH):   # For each category
        ...
        with open('cache/'+METHOD+'-'+cat+'.json', 'w') as file:
            json.dump(all_models, file)

        p_threads = []
        BLOCK = int(len(all_models)/NUM_WORKERS)
        count=0
        for i in range(0, len(all_models), BLOCK):
            ...
            p = multiprocessing.Process(target=worker, args=(BATCH, METHOD, count, cat, i, i+BLOCK, PRED_PATH, GDTH_PATH, outfilename, ROT, TYPE))
            p.start()
            p_threads.append(p)
            count+=1
